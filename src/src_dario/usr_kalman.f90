...
! !pgi$l unroll = n:8
  ...
  DO k = S3p, N3p, strd
     DO j = S2p, N2p, strd
        DO i = S1p, N1p, strd
           if (x1p(i).ge.12.0.and.x1p(i).le.13.0 .and. x2p(j).ge.0.0.and.x2p(j).le.2.0 ) then
           n_data = n_data + 1
           DO ii = i, i+strd-1
              DO jj = j, j+strd-1
                 DO kk = k, k+strd-1
                    IF (ii.le.N1p .and. jj.le.N2p .and. kk.le.N3p) then
                            i_data(ii,jj,kk) = n_data
                            if (x2p(j).lt.0.5.or.x2p(j).gt.1.5) flag_data(ii,jj,kk) = 1 !comment it if you don't need it
                    END IF
                 END DO
              END DO
           END DO
  ...
  DO n = 1,n_data
     ...
     klmn%covar(1:intervals,1:klmn%m,1:3) = 1.0e6*dtime_out_kalm
     klmn%covar(1:intervals,1:klmn%m,4:6) = 1.0e3*dtime_out_kalm
  ...
  !--------------------------------------------------------------------------------------------------------
     ...
     klmn%obs_covar(1,1) = (1-klmn%flg)*c_stats(klmn%i_data,1) + klmn%flg*10.*c_stats(klmn%i_data,1)
     klmn%obs_covar(2,2) = (1-klmn%flg)*c_stats(klmn%i_data,2) + klmn%flg*10.*c_stats(klmn%i_data,2)
     klmn%obs_covar(3,3) = (1-klmn%flg)*c_stats(klmn%i_data,3) + klmn%flg*10.*c_stats(klmn%i_data,3)
     klmn%obs_covar(1,2) = (1-klmn%flg)*c_stats(klmn%i_data,4) + klmn%flg*10.*c_stats(klmn%i_data,4)
     klmn%obs_covar(1,3) = (1-klmn%flg)*c_stats(klmn%i_data,5) + klmn%flg*10.*c_stats(klmn%i_data,5)
     klmn%obs_covar(2,3) = (1-klmn%flg)*c_stats(klmn%i_data,6) + klmn%flg*10.*c_stats(klmn%i_data,6)
