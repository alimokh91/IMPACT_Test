...
def parse_args():
    ...
    parser.add_argument('--output', type=str, required=True, # default='bern_exp_metadata.json',
                    help='Output name of json file relative to directory containing experimental data')
...
if args.output[0] == '/' or args.output[-5:] != '.json':
    import pdb; pdb.set_trace()
    raise RuntimeError("The file {} must be relative to the data directory and end with '.json'. Exiting...".format(args.output, args.data))

# Read from input path the path to each phase folder and sort them
paths_to_phases = glob.glob('exp_data_all_timeslices/time*/')
