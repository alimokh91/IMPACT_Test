...
def main():
    ...
    parser.add_argument('--pulses', type=int, default=10,
                    help='Amount of heart pulses for data assimilation')
    ...
    parser.add_argument('--Lref', type=float, help='Reference length')
    parser.add_argument('--Uref', type=float, help='Reference velocity')
    ...
    # padding used to re-dimension the volume of investigation
    num_ext_mri_voxels = [((num_mri_voxels[i] - num_padding_voxels[i] + block_dims[i] -1) // block_dims[i]) * block_dims[i] for i in range(3)]
...
#             raise ValueError
    ...
    template_args['time_end']   = mri.time_heart_cycle_period *args.pulses  # 5000.
    ...
    template_args['dtime_out_kalm'] = mri.time_heart_cycle_period/(args.tr*len(mri.time))
    ...
    template_args['mu_fluid'] = os.path.realpath(args.mu)
    template_args['rho_fluid'] = os.path.realpath(args.rho)
    template_args['L_ref'] = os.path.realpath(args.Lref)
    template_args['U_ref'] = os.path.realpath(args.Uref)
