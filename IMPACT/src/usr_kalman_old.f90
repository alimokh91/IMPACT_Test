...
! !pgi$l unroll = n:8
  ...
  TYPE(kalman_t), pointer :: klmn
  INTEGER :: i, j, k, ii, jj, kk, n, m, mm
  INTEGER, DIMENSION(2,3) :: bounds
  REAL, DIMENSION(b1L:(N1+b1U),b2L:(N2+b2U),b3L:(N3+b3U)) :: work11, work22, work33
  ...
  if (phase.eq.1) then
     ...
     CALL num_to_string(2,mod(write_kalm_count/intervals+1,24),count_char2)

     if (associated(mri_inst).eqv..true.) call mr_io_deallocate_dist_flow_mri_padded(mri_inst)
     nullify(mri_inst); allocate(mri_inst)
     mri_inst%domain_padding = kalman_domain_padding
  ...
  do while(associated(klmn))
    ...
    covar_f(phase,:,:,:,5) = covar_f(phase,:,:,:,5) - mean_f(phase,:,:,:,1)*mean_f(phase,:,:,:,3)
    covar_f(phase,:,:,:,6) = covar_f(phase,:,:,:,6) - mean_f(phase,:,:,:,2)*mean_f(phase,:,:,:,3)
    if (write_kalm_count/intervals .eq. 0) then 
      covar_f(phase,:,:,:,1) = 1.0e-1 + mean_f(phase,:,:,:,1)*mean_f(phase,:,:,:,1)
      covar_f(phase,:,:,:,2) = 1.0e-1 + mean_f(phase,:,:,:,2)*mean_f(phase,:,:,:,2)
      covar_f(phase,:,:,:,3) = 1.0e-1 + mean_f(phase,:,:,:,3)*mean_f(phase,:,:,:,3)
      covar_f(phase,:,:,:,4) = 1.0e-2 + mean_f(phase,:,:,:,1)*mean_f(phase,:,:,:,2)
      covar_f(phase,:,:,:,5) = 1.0e-2 + mean_f(phase,:,:,:,1)*mean_f(phase,:,:,:,3)
      covar_f(phase,:,:,:,6) = 1.0e-2 + mean_f(phase,:,:,:,2)*mean_f(phase,:,:,:,3)
    end if
  ...
  do while(associated(klmn))
    ...
    DO kk = (bounds(1,3)-1)*kalman_num_spatial_refinements(3)+1,bounds(2,3)*kalman_num_spatial_refinements(3)+1
      DO jj = (bounds(1,2)-1)*kalman_num_spatial_refinements(2)+1,bounds(2,2)*kalman_num_spatial_refinements(2)+1
        DO ii = (bounds(1,1)-1)*kalman_num_spatial_refinements(1)+1,bounds(2,1)*kalman_num_spatial_refinements(1)+1
          ...
          do j = 1,3*klmn%m
            klmn%PfHt(3*n+1,j) = covar_f(phase,ii,jj,kk,1)*klmn%obs_oper(j,3*n+1) + &
                                 covar_f(phase,ii,jj,kk,4)*klmn%obs_oper(j,3*n+2) + &
                                 covar_f(phase,ii,jj,kk,5)*klmn%obs_oper(j,3*n+3)

            klmn%PfHt(3*n+2,j) = covar_f(phase,ii,jj,kk,4)*klmn%obs_oper(j,3*n+1) + &
                                 covar_f(phase,ii,jj,kk,2)*klmn%obs_oper(j,3*n+2) + &
                                 covar_f(phase,ii,jj,kk,6)*klmn%obs_oper(j,3*n+3)

            klmn%PfHt(3*n+3,j) = covar_f(phase,ii,jj,kk,5)*klmn%obs_oper(j,3*n+1) + &
                                 covar_f(phase,ii,jj,kk,6)*klmn%obs_oper(j,3*n+2) + &
                                 covar_f(phase,ii,jj,kk,3)*klmn%obs_oper(j,3*n+3)
          end do
    ...
    DO m = 1, 3*klmn%m-1
      klmn%obs_covar(m+1:3*klmn%m,m) = klmn%obs_covar(m,m+1:3*klmn%m)
    END DO
    !----------------------------------------------------------------------------------------------
    ...
    !----------------------------------------------------------------------------------------------
    CALL kalman_gain(klmn%PfHt,klmn%obs_covar,klmn%obs_data,klmn%K,size(klmn%obs_oper,2),size(klmn%obs_oper,1))
    !----------------------------------------------------------------------------------------------
    ...
    DO kk = (bounds(1,3)-1)*kalman_num_spatial_refinements(3)+1,bounds(2,3)*kalman_num_spatial_refinements(3)+1
      DO jj = (bounds(1,2)-1)*kalman_num_spatial_refinements(2)+1,bounds(2,2)*kalman_num_spatial_refinements(2)+1
        DO ii = (bounds(1,1)-1)*kalman_num_spatial_refinements(1)+1,bounds(2,1)*kalman_num_spatial_refinements(1)+1
         ...
         work1(ii,jj,kk) = klmn%K(3*n+1,1)/(dtime*aRK(substep))
         work2(ii,jj,kk) = klmn%K(3*n+2,1)/(dtime*aRK(substep))
         work3(ii,jj,kk) = klmn%K(3*n+3,1)/(dtime*aRK(substep))
  ...
  !--- worki(:,:,:) --> vel(:,:,:,i) -------------------------------------------------------------------------
  CALL interpolate2_pre_vel(.TRUE.,1,work1,work11(:,:,:))
  CALL interpolate2_pre_vel(.TRUE.,2,work2,work22(:,:,:))
  CALL interpolate2_pre_vel(.TRUE.,3,work3,work33(:,:,:))
  nl(:,:,:,1) = nl(:,:,:,1) - work11(:,:,:)
  nl(:,:,:,2) = nl(:,:,:,2) - work22(:,:,:)
  nl(:,:,:,3) = nl(:,:,:,3) - work33(:,:,:)
  !===========================================================================================================
  ...
  real  :: work(m)
  integer :: i,IPIV(m),INFO
  !integer :: ITER
  !real  :: swork(m*(m+1)),work_2(m,1),work(m,1)

  ! work_2 = (H*Sigma*transpose(H)+R)^-1*(d-H*mu_f)
  !call dsgesv(m,1,R,m,IPIV,d,m,work_2,m,work,swork,ITER,INFO)
