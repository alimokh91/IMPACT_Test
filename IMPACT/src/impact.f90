...
PROGRAM impact
  ...
  !write(*,*)"Test init alarm start"
  !CALL init_alarm
  !write(*,*)"Test init alarm end"

  !--- Set configuration / topology --------------------------------------------------------------------------
  !write(*,*)"Test config start"
  CALL configuration
  !write(*,*)"Test config end"

  !--- Test of input parameters ------------------------------------------------------------------------------
  !write(*,*)"Test param start"
  CALL test_parameter
  !write(*,*)"Test param end"
  ...
  Re = U_ref*L_ref*rho_fluid/mu_fluid  
  ...
  !OPEN(88, FILE='pressure_pulse_test'//restart_char//'.txt', STATUS='UNKNOWN')
