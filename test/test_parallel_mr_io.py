...
#   Run test
    fort_command = "fortran/%s %d %d %d %s 1> %s 2> %s" % \
...
def validate_dist_spacetime_fort_array(test_inst, mpi_rank, array, out_lines, transpose_dims):
    ...
    fort_array = np.fromstring(out_lines[7 if len(transpose_dims) > 4 else 6], dtype=float, sep=' ').reshape(np.flip(np.concatenate((fort_vector_dims, np.array([fort_time_dim-fort_time_offset]), fort_dims)))).transpose(transpose_dims)
...
class TestHPCPredictMRI(unittest.TestCase): # FIXME: coordinates test...
    ...
    # Filenames
    filename_prefix = "mr_io_test_parallel_reader_hpc_predict"
...
class TestSegmentedHPCPredictMRI(unittest.TestCase): # FIXME: coordinates test...
    ...
    # Filenames
    filename_prefix = "mr_io_test_parallel_reader_segmented_hpc_predict"
