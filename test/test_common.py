...
def fortran_strace(cls): # can be used as a prefix to the Fortran command to trace system calls
    if not inspect.isclass(cls):
        cls = type(cls)
    if cls.__module__.startswith('test_sequential'):
        return " strace -o " + get_test_filename(cls, "log.strace") + " "
    elif cls.__module__.startswith('test_parallel') or cls.__module__.startswith('test_impact'):
        return " strace -o " + get_test_filename(cls, "log_%d.strace") % get_mpi_rank() + " "
    else:
        raise ValueError("{} should be a class from either test_sequential or test_parallel/test_impact module".format(cls))


def fortran_exec(test_cls):
    return test_cls.fortran_exec


def get_test_filename(cls, filename, relative_to_cache_folder=False):
    relative_path = os.path.join(cls.__module__, cls.__name__, filename)
    #relative_path = '_'.join([cls.__module__, cls.__name__]) + filename_ending
    if not relative_to_cache_folder:
        path = os.path.join(os.environ['CI_CACHE_FOLDER'], relative_path)
    else:
        path = relative_path
#    os.makedirs(os.path.dirname(path), exist_ok=True)  # potentially needs to be made outside container
    return path

def filename_mri(cls, relative_to_cache_folder=False):
    if not inspect.isclass(cls):
        cls = type(cls)
    return get_test_filename(cls, 'mri.h5', relative_to_cache_folder)


def filename_mri_in(cls, relative_to_cache_folder=False):
    if not inspect.isclass(cls):
        cls = type(cls)
    return get_test_filename(cls, 'mri_in.h5', relative_to_cache_folder)


def filename_mri_out(cls, relative_to_cache_folder=False):
    if not inspect.isclass(cls):
        cls = type(cls)
    return get_test_filename(cls, 'mri_out.h5', relative_to_cache_folder)


# def get_config_filename(cls, relative_to_cache_folder=False):
#     relative_path = os.path.join(cls.__module__, cls.__name__, 'config.txt')
#     if not relative_to_cache_folder:
...
def filename_config(cls, relative_to_cache_folder=False):
    if not inspect.isclass(cls):
        cls = type(cls)
    if cls.__module__.startswith('test_impact'):
        return get_test_filename(cls, 'config.txt', relative_to_cache_folder)
    raise ValueError("{} should be a class from test_impact module".format(cls))
...
def filename_out(cls, relative_to_cache_folder=False):
    if not inspect.isclass(cls):
        cls = type(cls)
    if cls.__module__.startswith('test_sequential'):
        return get_test_filename(cls, 'log.out', relative_to_cache_folder)
    elif cls.__module__.startswith('test_parallel') or cls.__module__.startswith('test_impact'):
        return get_test_filename(cls, 'log_%s.out', relative_to_cache_folder)
    else:
        raise ValueError("{} should be a class from either test_sequential or test_parallel/test_impact module".format(cls))
...
def filename_err(cls, relative_to_cache_folder=False):
    if not inspect.isclass(cls):
        cls = type(cls)
    if cls.__module__.startswith('test_sequential'):
        return get_test_filename(cls, 'log.err', relative_to_cache_folder)
    elif cls.__module__.startswith('test_parallel') or cls.__module__.startswith('test_impact'):
        return get_test_filename(cls, 'log_%s.err', relative_to_cache_folder)
    else:
        raise ValueError("{} should be a class from either test_sequential or test_parallel/test_impact module".format(cls))
...
def fortran_out(cls):
    if not inspect.isclass(cls):
        cls = type(cls)
    if cls.__module__.startswith('test_sequential'):
        return filename_out(cls)
    elif cls.__module__.startswith('test_parallel') or cls.__module__.startswith('test_impact'):
        return filename_out(cls) % get_mpi_rank()
    else:
        raise ValueError("{} should be a class from either test_sequential or test_parallel/test_impact module".format(cls))
...
def fortran_err(cls):
    if not inspect.isclass(cls):
        cls = type(cls)
    if cls.__module__.startswith('test_sequential'):
        return filename_err(cls)
    elif cls.__module__.startswith('test_parallel') or cls.__module__.startswith('test_impact'):
        return filename_err(cls) % get_mpi_rank()
    else:
        raise ValueError("{} should be a class from either test_sequential or test_parallel/test_impact module".format(cls))
...
def remove_test_files(cls, relative_to_cache_folder=False):
    if not inspect.isclass(cls):
        cls = type(cls)
    if cls.__module__.startswith('test_sequential'):
        ...
        if not cls.__name__.endswith('Bidirectional'):
            ...
            files = [filename_mri_in(cls), filename_mri_out(cls)] + \
                    [filename_out(cls) % (mpi_rank) for mpi_rank in range(cls.mpi_proc)] + \
                    [filename_err(cls) % (mpi_rank) for mpi_rank in range(cls.mpi_proc)]
    elif cls.__module__.startswith('test_impact'):
        impact_logs = [get_test_filename(cls, f) for f in ["test_beta.txt", "test_multigrid_properties.txt"]]
        files = [filename_mri_in(cls)] + ([filename_mri_out(cls)] if not fortran_exec(cls).endswith('input') else []) + \
                [filename_config(cls)] + \
                [filename_out(cls) % ("config_writer"),
                 filename_err(cls) % ("config_writer")] + \
                [filename_out(cls) % (mpi_rank) for mpi_rank in range(cls.mpi_proc)] + \
                [filename_err(cls) % (mpi_rank) for mpi_rank in range(cls.mpi_proc)] + \
                impact_logs  # IMPACT log files that are written without asking
    else:
        raise ValueError("{} should be a class from either test_sequential or test_parallel/test_impact module".format(cls))
    ...
    # # Could remove directories as well, but this seems not needed
    # relative_path = os.path.join(cls.__module__, cls.__name__)
    # if not relative_to_cache_folder:
    #      path = os.path.join(os.environ['CI_CACHE_FOLDER'], relative_path)
    # else:
    #      path = relative_path
    # # TODO: try except pattern
    # os.rmdir(path)
...
def print_out(filename_out, rank=None):
    read_out(filename_out, rank)
