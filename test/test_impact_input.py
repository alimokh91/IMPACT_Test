            ...
            with open(filename_out,'r') as out:
                ...
                for i in range(3):
                    ...
                    self.assertTrue( np.allclose(impact_coord_array[::TestImpactInput.sr[i]], 
                                                 self.geometry_complement[i][ block_id[i]*dims_mem[i]:(block_id[i]+1)*dims_mem[i]+1 ], rtol=1e-14)) # FIXME: probably failing due to numerical error in y-coordinates in IMPACT!
