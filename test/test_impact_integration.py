...
def validate_impact_coordinates(test_inst):
    ...
    for mpi_rank in range(test_cls.mpi_proc):
        ...
        filename_out = test_cls.filename_out_rank % (mpi_rank)
        filename_err = test_cls.filename_err_rank % (mpi_rank)
        ...
        validate_file_path(test_inst, out_lines[out_metainfo_begin + 0].strip(), os.path.realpath(test_inst.filename_mri_in)) # kalman_mri_input_file_path
        validate_file_path(test_inst, out_lines[out_metainfo_begin + 1].strip(), os.path.realpath(test_inst.filename_mri_out)) # kalman_mri_output_file_path
...
class TestImpactInput(unittest.TestCase): # FIXME: coordinates test...
    ...
    mpi_proc = int(os.environ['HPC_PREDICT_IO_TEST_MPI_FORTRAN_PROCS']) # 2**2
    ...
    sr = [2, 2, 2]
    tr = 10
    config_template = os.environ['HPC_PREDICT_IO_SRC_PATH'] + '/python/config.txt.j2'
    config_output = os.path.dirname(os.path.realpath(os.environ['HPC_PREDICT_IO_TEST_MRI_PATH'])) + '/config.txt'
...
class TestImpactInputPadding(unittest.TestCase): # FIXME: coordinates test...
    ...
    # number of Fortran MPI processes
    mpi_proc = int(os.environ['HPC_PREDICT_IO_TEST_MPI_FORTRAN_PROCS'])  # FIXME: 7 !!
